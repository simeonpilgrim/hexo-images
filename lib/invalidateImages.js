...
const invalidateImages = (hexo) =>
    async function () {
        ...
        const allImagesSpecial = allImages
            .map((item) => {
                const imageFileName = `${path.basename(item.file, path.extname(item.file)).replace(/\.noresize$/i, '')}${path.extname(
                    item.file
                )}`.toLowerCase();
            ...
            for (const imagesPortion of groupIntoParallelPortions(filesToProcess, 9)) {
                await Promise.all(
                    imagesPortion.map(async ({ file, fileName, size, postFilePath, special }) => {
                        ...
                        const postFileName = postFilePath ? path.relative(source_dir, postFilePath).replace(/\\/g,'/') : undefined;
...
module.exports = invalidateImages;
